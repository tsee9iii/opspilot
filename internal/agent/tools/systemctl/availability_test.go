...
func TestSystemCtlStatusToolAvailability(t *testing.T) {
	t.Run("available", func(t *testing.T) {
		...
		if binary != "systemctl" {
			t.Fatalf("expected check of systemctl binary, got %q", binary)
		}
	})
	...
	t.Run("not installed", func(t *testing.T) {
		...
		ok, reason := tool.Availability(context.Background())
		if ok || reason != "systemctl is not installed" {
	...
	t.Run("not runnable", func(t *testing.T) {
		...
		ok, reason := tool.Availability(context.Background())
		if ok || reason != "systemctl is not runnable" {
			t.Fatalf("expected unavailable, got ok=%v reason=%q", ok, reason)
		}
	})
}
